...
function determineFrontendUrl(req: NextRequest): string {
    // Canonical production URL takes priority — the sitemap should
    // always advertise internetkeeda.com regardless of which host
    // Google crawls (the Vercel preview URL, etc.).
    const env = process.env.NEXT_PUBLIC_SITE_URL || process.env.FRONTEND_URL;
    if (env) return env.replace(/\/$/, '');

    // Production fallback — canonical domain.
