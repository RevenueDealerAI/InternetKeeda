...
const getActualDbIdFromSponsored = (listing: SponsoredListing): string | null => {
  ...
  if (typeof toolIdField === 'object' && toolIdField !== null && toolIdField._id) {
    const populatedTool = toolIdField;
...
export const AIToolDetail = () => {
  ...
  const fetchAndSetTool = useCallback(async () => {
    ...
    try {
      ...
      if (sponsoredListingData) {
        ...
        if (underlyingDbId) {
          toolToFetchId = underlyingDbId;
          isResolvedSponsored = true;
        } else {
          // If sponsored listing exists but no valid underlyingDbId, fallback to fetching by slug
          // This handles cases where the sponsored listing exists but the tool reference is broken
        }
      }

      console.log(`[AIToolDetail] Fetching tool: ${toolToFetchId}`);
      const response = await fetch(`${API_BASE_URL}/api/tools/${toolToFetchId}`);
      if (!response.ok) {
        if (isResolvedSponsored && toolToFetchId !== slug) {
          const fallbackResponse = await fetch(`${API_BASE_URL}/api/tools/${slug}`);
          if (fallbackResponse.ok) {
            const fallbackToolData = await fallbackResponse.json() as Tool;
            if (fallbackToolData) {
              fallbackToolData.slug = slug;
              setTool(fallbackToolData);
              setIsLoading(false);
              return;
            }
          }
      ...
      if (fetchedToolData) {
        ...
        console.log(`[AIToolDetail] Tool fetched successfully:`, fetchedToolData.name);
  ...
  useEffect(() => {
    if (!sponsoredListingsLoading) {
      fetchAndSetTool();
    } else {
      console.log('[AIToolDetail] Waiting for sponsored listings to load before fetching tool...');
    }
  ...
  return (
    <motion.div
      className="theme-two min-h-screen bg-white overflow-x-hidden"
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
                ...
                >
                  ...
                  <div className="flex-1">
                    <div className="flex flex-col gap-6">
                      {/* Back button - Desktop at top, Mobile removed from here */}
                      <div className="hidden sm:block">
                        <Link href="/"
                          className="inline-flex items-center gap-2 px-4 py-2 text-sm text-gray-600 hover:text-gray-900 bg-white hover:bg-gray-50 rounded-full border border-gray-200 hover:border-purple-300 transition-all duration-200 group shadow-sm"
                        >
                      ...
                      <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6 sm:gap-6">
                        ...
                        <div className="relative">
                          <div className="w-28 h-28 sm:w-24 sm:h-24 rounded-2xl overflow-hidden bg-gradient-to-br from-purple-100 to-pink-50 shadow-lg ring-2 ring-purple-100 flex items-center justify-center relative">
                            <Image
                              src={(() => {
                              ...
                              onError={(e) => {
                                ...
                                // If current src is logo, try favicon
                                if (tool.logo && target.src === tool.logo) {
                                  try {
                                    const hostname = new URL(tool.websiteUrl).hostname;
                                    target.src = `https://www.google.com/s2/favicons?domain=${hostname}&sz=128`;
                                    return;
                                  } catch {
                                    // Ignore URL parsing errors - fallback will be handled below
                                  }
                                }
                                // If current src is favicon or logo failed fallback, go to ui-avatars
                          ...
                          {tool.isNew && (
                            <div className="absolute -bottom-3 -right-3 bg-gradient-to-r from-purple-500 to-pink-500 text-white px-3 py-1 rounded-full text-xs font-bold shadow-md">
                              NEW
                            </div>
                          )}
                        </div>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-6 sm:mt-8">
                      ...
                      <div className="bg-white rounded-xl p-3 sm:p-4 flex flex-col items-center text-center">
                        <Shield className="w-5 h-5 sm:w-6 sm:h-6 text-purple-500 mb-1" />
                    ...
                    <div className="grid grid-cols-2 sm:flex sm:items-center gap-3 mt-6 sm:mt-8">
                      <Button
                        ...
                        style={{
                          background: (currentActualDbId && isUpvoted(currentActualDbId))
                            ? 'linear-gradient(90deg, #8039fd 0%, #f5a5ad 100%)'
                            : 'linear-gradient(90deg, #8039fd 0%, #f5a5ad 100%)',
                        }}
                        className={`w-full sm:flex-1 sm:max-w-[200px] rounded-full transition-all duration-300 text-white hover:shadow-lg ${(currentActualDbId && isUpvoted(currentActualDbId))
                          ? "opacity-90"
                          : ""
                          }`}
                      >
                      ...
                      <Button
                        variant="outline"
                        size="default"
                        className={`w-full sm:flex-1 sm:max-w-[200px] rounded-full ${(currentActualDbId && isSaved(currentActualDbId)) ? "bg-purple-50 border-purple-200 text-purple-700" : ""
                          }`}
                      ...
                      <Button
                        variant="outline"
                        size="default"
                        className="w-full sm:flex-1 sm:max-w-[200px] rounded-full"
                        onClick={handleShare}
                      >
                        ...
                        <div className="overflow-x-auto">
                          <TabsList className="w-full justify-start bg-transparent border-b rounded-none p-0 h-auto">
                            <div className="flex w-full min-w-max">
                              <TabsTrigger
                                value="about"
                                className="flex-1 min-w-[80px] data-[state=active]:bg-transparent data-[state=active]:shadow-none data-[state=active]:border-b-2 data-[state=active]:border-purple-600 rounded-none py-3 px-4 text-sm"
                              >
                                About
                              </TabsTrigger>
                              <TabsTrigger
                  ...
                  <div className="w-full lg:w-80 mt-6 lg:mt-0 space-y-6">
                    {/* Highlights Card */}
                    <div className="bg-gray-50 rounded-xl sm:rounded-2xl p-4 sm:p-6 border border-gray-200 hover:border-purple-200 transition-colors">
                      <h3 className="text-sm font-semibold text-gray-900 mb-3 sm:mb-4">Highlights</h3>
                      <div className="space-y-3">
                        {tool.features.slice(0, 4).map((feature, index) => (
                          <div key={index} className="flex items-center gap-2 text-sm sm:text-base text-gray-600">
                            ...
                            <span>{feature.replace(/✅|✔️|✓/g, '').trim()}</span>
                          </div>
                        ))}
                      </div>
                    </div>
                    ...
                    {/* Tags Card */}
                    <div className="bg-gray-50 rounded-xl sm:rounded-2xl p-4 sm:p-6 border border-gray-200 hover:border-purple-200 transition-colors">
                      <h3 className="text-sm font-semibold text-gray-900 mb-3 sm:mb-4">Tags</h3>
                      <div className="flex flex-wrap gap-2">
                        {tool.tags.map((tag, index) => (
                          <Badge
                            key={index}
                    ...
                    {/* Links Card */}
                    <div className="bg-gray-50 rounded-xl sm:rounded-2xl p-4 sm:p-6 border border-gray-200 hover:border-purple-200 transition-colors">
                      <h3 className="text-sm font-semibold text-gray-900 mb-3 sm:mb-4">Links</h3>
                      <div className="space-y-3">
                        <a
        ...
        {showQuickActions && (
          ...
          >
            <Button
              size="icon"
              variant="outline"
              className="h-12 w-12 rounded-xl bg-white shadow-lg hover:bg-purple-50 hover:border-purple-300 transition-colors"
              onClick={scrollToTop}
            >
