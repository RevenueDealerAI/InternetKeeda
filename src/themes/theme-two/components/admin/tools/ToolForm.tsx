...
export function ToolForm({ initialData, onSubmit, onCancel }: ToolFormProps) {
  ...
  const form = useForm<ToolFormData>({
    ...
    defaultValues: initialData || {
      ...
      category: "",
      tags: [],
      pricing: { type: "free", startingPrice: undefined },
      features: [],
      status: "draft",
      isTrending: false,
      isNew: false,
  ...
  const inputClass = 'rounded-full focus-visible:ring-2 focus-visible:ring-purple-500 focus-visible:border-purple-500';
  const textareaClass = 'rounded-xl focus-visible:ring-2 focus-visible:ring-purple-500 focus-visible:border-purple-500';
  const selectTriggerClass = 'rounded-full focus-visible:ring-2 focus-visible:ring-purple-500';
  const primaryBtnClass = 'rounded-full bg-purple-600 text-white hover:bg-purple-700';
  const outlineBtnClass = 'rounded-full';
  const checkboxClass = 'border-purple-400 focus-visible:ring-purple-500 data-[state=checked]:bg-purple-600';
  const tagContainerClass = 'rounded-full focus-within:ring-2 focus-within:ring-purple-500 focus-within:border-purple-500';
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Input {...field} placeholder="Enter tool name" className={inputClass} />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Textarea {...field} placeholder="Enter tool description" className={`min-h-[100px] ${textareaClass}`} />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Input {...field} placeholder="https://example.com" type="url" className={inputClass} />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <Input {...field} placeholder="https://example.com/logo.png" type="url" className={inputClass} />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <TagInput value={field.value} onChange={field.onChange} suggestions={Array.from(COMMON_TAGS)} placeholder="Add tags..." containerClassName={tagContainerClass} />
              </FormControl>
              <FormMessage />
            </FormItem>
          )}
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <Input type="number" {...field} value={field.value || ''} onChange={(e) => { const value = e.target.value === '' ? undefined : parseFloat(e.target.value); field.onChange(value); }} placeholder="0.00" className={inputClass} />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
        ...
        <FormField
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <FormControl>
                <TagInput value={field.value} onChange={field.onChange} placeholder="Add features (press Enter or comma to add)..." containerClassName={tagContainerClass} />
              </FormControl>
          ...
          <div className="grid grid-cols-2 gap-4">
            <FormField control={form.control} name="isTrending" render={({ field }) => (
              <FormItem className="flex items-center gap-3">
                <FormControl>
                  <Checkbox checked={field.value} onCheckedChange={field.onChange} className={checkboxClass} />
                </FormControl>
                <div>
            ...
            <FormField control={form.control} name="isTopRated" render={({ field }) => (
              <FormItem className="flex items-center gap-3">
                <FormControl>
                  <Checkbox checked={field.value} onCheckedChange={field.onChange} className={checkboxClass} />
                </FormControl>
        ...
        <div className="flex justify-end space-x-4">
          <Button variant="outline" onClick={onCancel} className={outlineBtnClass}>Cancel</Button>
          <Button type="submit" className={primaryBtnClass}>{initialData ? "Update Tool" : "Create Tool"}</Button>
        </div>
      </form>
    </Form>
  );
